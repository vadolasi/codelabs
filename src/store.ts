...
export interface Actions {
  ...
  swapOpenedFiles: (workspaceId: string, index1: number, index2: number) => void
}
...
export const useStore = create(immer<State & Actions>(set => ({
  ...
  addFile: (id, workspaceId, parent, name, type, children = []) => set(state => {
    ...
    if (type === "file" && !workspace.openedFiles.includes(id)) {
  ...
  swapOpenedFiles: (workspaceId, index1, index2) => set(state => {
    const workspace = state.workspaces[workspaceId]

    const temp = workspace.openedFiles[index1]

    workspace.openedFiles[index1] = workspace.openedFiles[index2]
    workspace.openedFiles[index2] = temp
  })
})))
