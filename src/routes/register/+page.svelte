...
import { arktype, zod, zodClient } from "sveltekit-superforms/adapters"
import { z } from "zod"
...
<div class="flex w-fulll min-h-screen items-center justify-center">
  <div class="card card-border card-lg bg-base-100 shadow-sm w-full m-10 md:m-0 md:w-2/3 lg:w-1/3 xl:w-1/4">
    <form class="card-body space-y-3" use:enhance novalidate>
      ...
      <FormField {form} field="email" label="Email" type="email" autocomplete="email" inputmode="email" required>
        {#if emailSuggestion}
      ...
      <FormField {form} field="username" label="Nome de usuário" type="text" autocomplete="username" required />
      <FormField {form} field="password" label="Senha" type="password" autocomplete="new-password" required containerClass={cn(score !== null && "mb-0")}>
        {#if score !== null}
      ...
      <FormField {form} field="passwordConfirmation" label="Confirme a senha" type="password" autocomplete="new-password" required />
